...
class TargetTracker:
    def __init__(self, target_id):
        ...
        # 彈孔偵測器 (針對此靶)
        self.shot_detector = WarpedShotDetector()
        self.shot_detector_initialized = False

        # IDPA 計分結果暫存
        self.all_holes = []  # [(x, y, shot_index), ...]
...
# 計分面板繪製
# ==========================================
def draw_score_panel(target_w, target_h, tracker: TargetTracker, scorer: IDPAScorer):
    """繪製計分面板，顯示每發擊中資訊與總計分。"""
    ...
    # 標題
    cv2.putText(panel, f"Target ID:{tracker.target_id} Score",
                (10, y_cursor), cv2.FONT_HERSHEY_SIMPLEX, 0.6, tracker.color, 2)
    y_cursor += line_h + 5
...
def run_pipeline():
    ...
    while True:
        ...
        for t in active_trackers:
            ...
            if M is not None:
                ...
                if is_valid_candidate:
                    # ── 初始化彈孔偵測器（用第一幀 TPS 結果作為基準） ──
                    if not t.shot_detector_initialized:
                        t.shot_detector.init_reference(temp_perfect)
                        t.shot_detector_initialized = True
                        print(f"🔫 Target ID:{t.target_id} 彈孔偵測器初始化完成")
                    else:
                        # ── 在攤平影像上偵測彈孔 ──
                        new_holes = t.shot_detector.detect(temp_perfect)
                        if new_holes:
                            for (hx, hy, shot_idx) in new_holes:
                                t.all_holes.append((hx, hy, shot_idx))
                                zone = scorer.target.get_hit_zone(hx, hy)
                                penalty = ZONE_PENALTY.get(zone, 5)
                                zone_str = f"-{zone}" if isinstance(zone, int) else "Miss"
                                print(f"🎯 Target ID:{t.target_id} 第 {shot_idx} 發 @ ({hx},{hy}) "
                                      f"-> {zone_str} (+{penalty}s)")
                    ...
                    # 繪製所有彈孔標註
                    draw_holes_on_warped(scored_perfect, t.all_holes, scorer)

                    t.warped_frame = temp_warped
                    t.perfect_warped_frame = scored_perfect
                    t.last_valid_org_12_points = org_12_points

                    cv2.putText(t.perfect_warped_frame, f"ID:{t.target_id} Scoring",
                                (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, t.color, 2)
        ...
        # 每個靶的顯示區: warped + scored_perfect + debug_1(ref, cand) + debug_2(diff, mask) + score_panel
        block_w = target_w * 5
        block_h = target_h
        ...
        if num_targets > 0:
            ...
            for i, t in enumerate(calibrated_trackers):
                ...
                if t.warped_frame is not None and t.perfect_warped_frame is not None:
                    # 第 1 欄: Homography warped
                    grid_view[y_offset:y_offset + block_h,
                              x_offset:x_offset + target_w] = t.warped_frame
                    # 第 2 欄: TPS + 計分區 + 彈孔標註
                    grid_view[y_offset:y_offset + block_h,
                              x_offset + target_w:x_offset + target_w * 2] = t.perfect_warped_frame
                              
                    # 第 3 欄: Ref (上) / Candidates (下)
                    ref_gray = t.shot_detector.debug_frames.get('ref', np.zeros((target_h, target_w), dtype=np.uint8))
                    ref_bgr = cv2.cvtColor(ref_gray, cv2.COLOR_GRAY2BGR)
                    ref_resized = cv2.resize(ref_bgr, (target_w, half_h))
                    cv2.putText(ref_resized, "Ref Frame", (5, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1)
                    
                    cand_bgr = t.shot_detector.debug_frames.get('candidates', np.zeros((target_h, target_w, 3), dtype=np.uint8))
                    cand_resized = cv2.resize(cand_bgr, (target_w, target_h - half_h))
                    cv2.putText(cand_resized, "Candidates", (5, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1)
                    
                    grid_view[y_offset:y_offset + half_h,
                              x_offset + target_w * 2:x_offset + target_w * 3] = ref_resized
                    grid_view[y_offset + half_h:y_offset + target_h,
                              x_offset + target_w * 2:x_offset + target_w * 3] = cand_resized

                    # 第 4 欄: Diff (上) / Mask (下)
                    diff_gray = t.shot_detector.debug_frames.get('diff', np.zeros((target_h, target_w), dtype=np.uint8))
                    diff_bgr = cv2.cvtColor(diff_gray, cv2.COLOR_GRAY2BGR)
                    diff_resized = cv2.resize(diff_bgr, (target_w, half_h))
                    cv2.putText(diff_resized, "Diff (Ref - Cur)", (5, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1)
                    
                    mask_gray = t.shot_detector.debug_frames.get('mask', np.zeros((target_h, target_w), dtype=np.uint8))
                    mask_bgr = cv2.cvtColor(mask_gray, cv2.COLOR_GRAY2BGR)
                    mask_resized = cv2.resize(mask_bgr, (target_w, target_h - half_h))
                    cv2.putText(mask_resized, "Thresh & Morph", (5, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1)
                    
                    grid_view[y_offset:y_offset + half_h,
                              x_offset + target_w * 3:x_offset + target_w * 4] = diff_resized
                    grid_view[y_offset + half_h:y_offset + target_h,
                              x_offset + target_w * 3:x_offset + target_w * 4] = mask_resized

                    # 第 5 欄: 計分面板
                    score_panel = draw_score_panel(target_w, target_h, t, scorer)
                    grid_view[y_offset:y_offset + block_h,
                              x_offset + target_w * 4:x_offset + target_w * 5] = score_panel
            ...
            empty_grid = np.zeros((target_h, target_w * 5, 3), dtype=np.uint8)
    ...
    for t in [t for t in active_trackers if t.is_calibrated]:
        ...
        print(f"   偵測到 {len(t.all_holes)} 發")

        if t.all_holes:
            hits_xy = [(x, y) for x, y, _ in t.all_holes]
            result = scorer.score_single_target(hits_xy, shots_required=2)

            for (x, y, shot_idx) in t.all_holes:
                zone = scorer.target.get_hit_zone(x, y)
                penalty = ZONE_PENALTY.get(zone, 5)
                zone_str = f"-{zone}" if isinstance(zone, int) else "Miss"
                print(f"   第 {shot_idx:2d} 發: ({x:3d},{y:3d}) -> {zone_str:>5s} (+{penalty}s)")

            print(f"   ────────────────")
            print(f"   最佳 2 發罰秒: {result['best_hits_points_down']}")
            print(f"   單靶 Points Down: +{result['total_target_penalty']}s")

            # 儲存最終計分靶圖
            if t.perfect_warped_frame is not None:
                save_path = os.path.join(output_dir, f"target_{t.target_id}_scored.png")
                cv2.imwrite(save_path, t.perfect_warped_frame)
                print(f"   靶圖已儲存: {save_path}")
        ...
        for (x, y, shot_idx) in t.all_holes:
            ...
            hole_data.append({
                "shot_index": shot_idx,
                "x": x, "y": y,
                "zone": zone if isinstance(zone, int) else "Miss",
                "penalty_seconds": ZONE_PENALTY.get(zone, 5)
            })

        json_path = os.path.join(output_dir, f"target_{t.target_id}_holes.json")
        with open(json_path, "w", encoding="utf-8") as f:
            json.dump({
                "target_id": t.target_id,
                "total_shots": len(t.all_holes),
                "holes": hole_data,
                "timestamp": datetime.now().isoformat()
            }, f, indent=2, ensure_ascii=False)
        print(f"   資料已儲存: {json_path}")
